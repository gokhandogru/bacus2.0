...
$sql = "SELECT * FROM bacus_multilingual WHERE Catalan = '$keywordfromform' OR Catalan LIKE '% $keywordfromform' OR Catalan LIKE '$keywordfromform %' OR Catalan LIKE '% $keywordfromform %' ORDER BY CHAR_LENGTH(Catalan) LIMIT 25";
...
if ($result->num_rows > 0) {
  ...
  <div class='card-panel orange lighten-5'>";
		...
		echo (!empty($row['Catalan_Context']) ? "<td><b>Catalan Context:</b><br>{$row["Catalan_Context"]}</td><br><hr>" : "");
...
Every project has been supervised by an expert on the subject and has been developed at least in three languages; chosen among the languages offered in our degree (Arabic, Catalan, Chinese, English, German, Italian, Japanese, Portuguese, Russian, and Spanish).</p>";
	...
	<br>
	<br>
	<center><b><a href="http://espanolturco.com/bacus/index_bacus_catalan.php">Return to Catalan term search</a></b></center>
	<?php
